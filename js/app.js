(() => {
    ...
    function digitСounter(selector, time = 1500, step = 5) {
        selector.forEach((element => {
            ...
            for (var index in el) if (Number(el[index]) || 0 == Number(el[index])) numEl += el[index]; else strEl += el[index];
            var number = parseInt(numEl);
            var n = 0;
            ...
            let timerId = setTimeout((function request() {
                ...
                if (n >= number) {
                    ...
                    element.innerHTML = n + strEl;
                    setTimeout(request, t);
                    t *= 1.03;
                }
            }), t);
        }));
    ...
    function lineRun(line) {
        ...
        line.forEach((element => {
            var el = parseInt(element.getAttribute("data-line"));
            let t = Math.round(time / (el / step));
            var n = 0;
            let timerId = setTimeout((function request() {
                element.style.opacity = "1";
                n += step;
                if (n >= el) {
                    element.style.width = el + "%";
                    clearTimeout(timerId);
                } else {
...
*/
    (function(global, factory) {
        ...
        !function() {
            ...
            var t = [ "-" ];
            var n = "2023-2-10".split("-"), e = new Date(n[0], n[1], n[2]), r = [ "se", "licen", "-", "v3", "l", "gp" ];
            function i() {
    ...
    pageSlider.on("slideNextTransitionEnd", (function() {
        let listElement = pageSlider.slides[pageSlider.realIndex].querySelectorAll("[data-digit-counter]");
        if (listElement.length > 0) digitСounter(listElement, 500, 5);
        let listLine = pageSlider.slides[pageSlider.realIndex].querySelectorAll("[data-line]");
        if (listElement.length > 0) lineRun(listLine);
    }));
    addLoadedClass();
})();